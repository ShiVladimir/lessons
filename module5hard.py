...
#где будут размещаться дополнительные полезные ролики
#на тему IT (юмористические, интервью и т.д.).
#Конечно же для старта написания интернет ресурса требуются
# хотя бы базовые знания программирования.
...
#    то current_user меняется на найденного. Помните,
...
#    Метод add, который принимает неограниченное
#    кол-во объектов класса Video и все добавляет в videos,
...
#    Следует учесть, что слово 'UrbaN' присутствует в
#    строке 'Urban the best' (не учитывать регистр).
...
#    Воспроизводить видео можно только тогда,
#    когда пользователь вошёл в UrTube.
#    В противном случае выводить в консоль надпись:
#    "Войдите в аккаунт, чтобы смотреть видео"
#    Если видео найдено, следует учесть,
#    что пользователю может быть отказано в просмотре,
...
#    Не забывайте для удобства использовать
#    dunder(магические) методы: __str__, __repr__, __contains__, __eq__ и др.
#    (повторить можно здесь)
#    Чтобы не запутаться рекомендуется после реализации
#    каждого метода проверять как он работает, тестировать разные вариации.


#Файл с кодом (module5hard.py) прикрепите к
# домашнему заданию или пришлите ссылку на ваш GitHub репозиторий с файлом решения.
